...
// Call ML API for prediction (server-side)
export const callMLAPI = action({
  args: {
    age: v.number(),
    bmi: v.number(),
    glucose: v.number(),
    bloodPressure: v.optional(v.number()),
    insulin: v.optional(v.number()),
    skinThickness: v.optional(v.number()),
    pregnancies: v.optional(v.number()),
    familyHistory: v.optional(v.number()),
    ...
    familyHistoryFlag: v.optional(v.boolean()),
  ...
  handler: async (ctx, args) => {
    ...
    if (!mlApiUrl) {
      throw new Error("ML_API_URL environment variable is not set");
    }
    ...
    const requestData = {
      ...
      bloodPressure: args.bloodPressure ?? (args.systolicBP && args.diastolicBP ? (args.systolicBP + args.diastolicBP) / 2 : undefined),
      insulin: args.insulin,
      skinThickness: args.skinThickness,
      pregnancies: args.pregnancies ?? 0,
      familyHistory: args.familyHistory ?? (args.familyHistoryFlag ? 1 : 0),
      gender: args.gender,
      systolicBP: args.systolicBP,
      diastolicBP: args.diastolicBP,
      heartRate: args.heartRate,
      hba1c: args.hba1c,
      exerciseFrequency: args.exerciseFrequency,
      smokingStatus: args.smokingStatus,
      alcoholConsumption: args.alcoholConsumption,
      familyHistoryFlag: args.familyHistoryFlag,
      diabetesStatus: args.diabetesStatus,
    };

    try {
      // Ensure URL doesn't have trailing slash and has proper protocol
      const baseUrl = mlApiUrl.replace(/\/$/, ""); // Remove trailing slash
      ...
      if (error instanceof Error) {
        if (error.name === "AbortError") {
          throw new Error("ML API request timed out. Please try again.");
        }
        if (error.message.includes("fetch failed") || error.message.includes("NetworkError")) {
          throw new Error(
            `Cannot connect to ML API at ${apiUrl}. Please check that the ML_API_URL environment variable is set correctly in Convex and that the ML API service is running.`
          );
        }
        throw new Error(
          `Failed to get prediction from ML API: ${error.message}`
        );
      }
      throw new Error("Failed to get prediction from ML API: Unknown error");
    }
  },
});
